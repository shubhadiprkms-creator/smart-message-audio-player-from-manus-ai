...
module.exports = function withSmartMessageAudio(config) {
  ...
  config = withMainApplication(config, (config) => {
    ...
    const withoutImport = contents.split(`${importLine}\n`).join("");
    const packageLine = withoutImport.match(/^package[^\n]*\n/m)?.[0] ?? "";
    let updated = packageLine ? withoutImport.replace(packageLine, `${packageLine}${importLine}\n`) : `${importLine}\n${withoutImport}`;
